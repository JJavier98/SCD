...
class MbarberiaSU : public HoareMonitor{
 ...
 std::vector<int> cola;/*EXAMEN 	Aquí se almacenarán, en orden de entrada, a los clientes */
...
void MbarberiaSU::cortarPelo(int c){
 	...
 	if(!silla.empty())
 	{
 		...
 		cola.push_back(c);/*EXAMEN 	 En orden de llegada se van introduciendo los clientes en el vector*/
  	...
  	else if (!activo.empty())
  	{
    	...
    	activo.signal();
  		...
  		/* EXAMEN*/
  		// Comprobamos que el cliente al que se le va a pelar el pelo es el primero por orden de entrada
  		// Si no se emite un mensaje de error
  		// Una vez pasa a pelarse se saca de la cola de "esperando"
  		if(cola.size() != 0)
  		{  		
  			if(c != cola[0])
