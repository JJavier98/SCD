...
class MbarberiaSU : public HoareMonitor{
  private:
  int num_clientes,
	  	contador; /* EXAMEN */ // Esta variable auxiliar nos ayudará a implementar que el barbero se duerma el 50% de las veces y se acicale el 50% restante
  CondVar activo,
          silla,
          sala_espera;
...
void MbarberiaSU::siguienteCliente(){	
   if(sala_espera.empty()){
   	/* EXAMEN */
    // Aumentamos el contador progresivamente, cuando sea par el barbero dormirá, cuando sea impar, el barbero se cortará el pelo a sí mismo. 50% - 50%
	   contador++;
	   contador = contador % 2;
   		if(contador == 0) 
