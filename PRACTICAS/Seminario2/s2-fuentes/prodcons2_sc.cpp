...
{
   int contador = num_hebra*(num_items/num_prod) + valores_producidos[num_hebra] ;

   this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));
   mtx.lock();
   cout << "producido: " << contador << " por " << num_hebra << endl << flush ;
   valores_producidos[num_hebra]++;
   mtx.unlock();
   cont_prod[contador] ++ ;
   return contador ;
}
...
{
   for( unsigned i = num_hebra*(num_items/num_prod) ; i < (num_items/num_prod) + ((num_items/num_prod)*num_hebra) ; i++ )
   {
