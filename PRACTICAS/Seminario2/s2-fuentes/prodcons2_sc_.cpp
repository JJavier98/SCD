...
int producir_dato(int i)
{
   ...
   bool  condicion1 = contador >= (i*num_items/np),
         condicion2 = contador < ( (i*num_items/np)  + (num_items/np) );

         if (condicion1)
            cout << "hola" << endl;
         if (condicion2)
         {
            cout << i*num_items/np  + num_items/np;
            cout << "hele" << endl;
         }

   while(! (condicion1 && condicion2) )
      productores.wait(guarda);


   cout << "producido: " << contador << " por hebra: " << i << endl << flush ;
   
   cont_prod[contador] ++ ;
   cont_hebra_prod[i] ++;
   contador++;
   cout << cont_hebra_prod[i] << " " << contador << endl;

   if(cont_hebra_prod[i] == num_items/np)
    {
		productores.notify_one();
		productores.wait(guarda);
	}
...
void consumir_dato( unsigned dato, int i )
{
   ...
   this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));
   mtx.lock();
   cout << "                  consumido: " << dato << " por hebra: " << i << endl ;
   mtx.unlock();
}
//----------------------------------------------------------------------
...
void funcion_hebra_productora( ProdCons1SC * monitor, int j )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
...
void funcion_hebra_consumidora( ProdCons1SC * monitor, int j )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      int valor = monitor->leer();
      consumir_dato( valor, j ) ;
   }
}
// -----------------------------------------------------------------------------
